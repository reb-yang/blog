...
Based on this, k=4 seems like a reasonable estimate, since we can see four clearly defined clusters. Things start to get murky with five, especially between the green and pink regions. This will make it easier to make distinctions between the different characters, which is our goal. 
...
- Cluster 4: the wholesome one (proper, cautious, soft, warm)
...
highlight(plot_inter, on = "plotly_click", off = "plotly_deselect", opacityDim = 0.1)
...
All in all, despite that being not the most rigorous analysis, I think that worked pretty well! This was a super fun data set to play around with, so again, thanks to Alex Cookson! In the unlikely case that the Open Psychometrics person is reading this: if you add the Bridgerton characters, that would be the highlight of my year. 
